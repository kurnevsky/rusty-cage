...
#[derive(Clone, Debug)]
pub enum SeccompPanic {
  SetNoNewPrivsFailed(c_int),
  SetDumpableFlagFailed(c_int),
  SetSeccompFilterFailed(c_int),
...
fn set_no_new_privs() {
  ...
  if result != 0 {
    panic!(SeccompPanic::SetNoNewPrivsFailed(result));
  }
}
...
fn set_dumpable() {
  ...
  if result != 0 {
    panic!(SeccompPanic::SetDumpableFlagFailed(result));
  }
}
...
fn build_program(filter_type: SeccompFilterType, syscalls_list: &[String]) -> Vec<sock_filter> {
  ...
  for syscall in syscalls_list {
    if let Some(number) = SYSCALLS.get(syscall.as_str()).cloned() {
      ...
      panic!(SeccompPanic::UnknownSyscallName(syscall.to_owned()));
    }
  }
...
fn set_seccomp_filter(filter_type: SeccompFilterType, syscalls_list: &[String]) {
  ...
  if result != 0 {
    panic!(SeccompPanic::SetSeccompFilterFailed(result));
  }
}
